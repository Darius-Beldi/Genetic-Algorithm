...
def mutation(cromozom, _domain_start, _domain_end):
    ...
    while not (_domain_start <= bits_to_float(cromozom) <= _domain_end):
        cromozom = list(cromozom)
        mutation_mask = [str(randomInt(0, 1)) for x in range(len(cromozom))]
        for i in range(len(cromozom)):
            if mutation_mask[i] == "1":
...
def mutation_for_all_chromosomes(_population, _mutation_probability, _a, _b, _c, generation, _domain_start, _domain_end):
    for i in range(len(_population)):
        rand = randomInt(0,100)
        if rand <= _mutation_probability:
            _population[i].binary = mutation(_population[i].binary, _domain_start, _domain_end)
